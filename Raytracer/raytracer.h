...
------------------------------------------------------------------------*/
void doRaytrace(int raysPerPixel = 4, int dw = 480, int dh = 272, int q = 1)
{
  // Trace it
  int dw2=dw/2;;
  int dh2=dh/2;
  ...
  for (int y=0; y<dh; y+=q) {
    for (int x=0; x<dw; x+=q) {
      ...
      if(q==1) tft.drawPixel(x, y, tft.color565(r,g,b));
      else tft.fillRect(x, y, q, q, tft.color565(r,g,b));
    }
    char buf[100];
